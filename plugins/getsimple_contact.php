...
function sc_frontend($content) {
    if (preg_match('#\[sc_form(.*)\]#', $content)) {
        if (isset($_POST) && $_POST) {
		   // -- check if subject is empty and call the function sc_get_parameter() to validate correct
		   // -- if a subject text was defined it will be filled in $_POST['subject'], if not, it will stay empty
           if (empty($_POST['subject'])) $_POST['subject'] = sc_get_parameter($content, 'subject');
...
// -- new function to read and catch parameters
// -- it is looking for double quotes behind the set_anything= 
function sc_get_parameter($content, $param) {
	$parameter = '';
	preg_match('#\[sc_form(.*)\]#', $content, $stringmatch);
	
	switch($param) {
	case 'subject' :
			if(strpos($stringmatch[1], 'set_subject=') > 0) {
				$new_string = substr($stringmatch[1], strpos($stringmatch[1], 'set_subject='));
				preg_match('/"([^"]+)"/', $new_string, $match);
				$parameter = trim($match[1], '"');
			}
			break;
	case 'template' :
			if(strpos($stringmatch[1], 'set_template=') > 0) {
				$new_string = substr($stringmatch[1], strpos($stringmatch[1], 'set_template='));
				preg_match('/"([^"]+)"/', $new_string, $match);
				$parameter = trim($match[1], '"');
			}
			break;
...
function sc_content($content, $error = null) {
    ...
    $template = sc_get_parameter($content, 'template');
	// -- new second parameter $template in the function call
    $form = sc_render_form($error, $template);
    $content = preg_replace('#\[sc_form(.*)\]#', $form, $content);
    return $content;
}

// -- new second parameter in the function
...
function sc_scripts() {
    ...
    $frontend->set('FE_CSS', '<link rel="stylesheet" type="text/css" href="' . $SITEURL . 'plugins/getsimple_contact/css/simple_contact.css" />');
    ...
    if ($check) {
        ...
        if (isset($data['attachment']) && count($data['attachment']) <= 5 && !empty($data['attachment']['name'])) {
            ...
            $allowed_type = array('text/plain','application/vnd.ms-excel','application/msword','application/vnd.openxmlformats-officedocument.wordprocessingml.document',
									...
									'application/x-zip','application/zip','application/x-zip-compressed','application/s-compressed',
									'multipart/x-zip','application/octet-stream','application/x-rar','application/rar','application/x-rar-compressed','application/octet-stream');
